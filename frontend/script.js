...
function updateStatus(elementId, text, statusClass = "") {
  if (elements[elementId]) {
    ...
    if (elementId === "connectionStatus") {
      elements[elementId].className = "connection-status " + statusClass;
    }
  }
}
...
async function connect() {
  try {
    ...
    state.client.automaticActivityDetection = {
      disabled: elements.disableActivityDetection?.checked ?? false,
      silence_duration_ms: parseInt(elements.silenceDuration?.value ?? "2000"),
      prefix_padding_ms: parseInt(elements.prefixPadding?.value ?? "500"),
      end_of_speech_sensitivity: elements.endSpeechSensitivity?.value ?? "END_SENSITIVITY_UNSPECIFIED",
      start_of_speech_sensitivity: elements.startSpeechSensitivity?.value ?? "START_SENSITIVITY_UNSPECIFIED",
    };

    // Set activity handling
    state.client.activityHandling = elements.activityHandling?.value ?? "ACTIVITY_HANDLING_UNSPECIFIED";
    ...
    if (!isGroundingEnabled) {
      ...
      // Add Memory Search Tool implicitly to give AURA memory access!
      const memoryTool = new SearchMemoryTool();
      state.client.addFunction(memoryTool);
      console.log("✅ Persistent Memory Access enabled");
    ...
    updateStatus("connectionStatus", "Connection failed", "status-disconnected");
...
function disconnect() {
  ...
  // Update UI
  updateStatus("connectionStatus", "Disconnected", "status-disconnected");
  elements.connectBtn.style.display = "flex";
  elements.disconnectBtn.style.display = "none";

  elements.startAudioBtn.textContent = "Mic";
  elements.startVideoBtn.textContent = "Camera";
  elements.startScreenBtn.textContent = "Screen";
  ...
  switch (message.type) {
    ...
    case MultimodalLiveResponseType.TOOL_CALL:
      ...
      for (let index = 0; index < functionCalls.length; index++) {
        ...
        let result;
        try {
          result = await state.client.callFunction(functionName, parameters);
...
// Connection handlers
function handleOpen() {
  updateStatus("connectionStatus", "Connected", "status-connected");
}
...
function addMessage(text, type, append = false) {
  ...
  if (append && lastMessage && lastMessage.classList.contains(className)) {
    // Append to existing message of the same type
    const contentSpan = lastMessage.querySelector(".message-content") || lastMessage;
    ...
    if (type !== "system") {
      ...
      label.textContent = type === "assistant" ? "Gemini" : "You";
...
// AURA Persistent Memory: Automatically store completed turns
function storeCurrentTurn() {
  const messages = elements.chatContainer.querySelectorAll(".chat-message");
  ...
  // Parse backwards to find the most recent user and ai messages
  for (let i = messages.length - 1; i >= 0; i--) {
     const msg = messages[i];
     const text = msg.querySelector(".message-content") ? msg.querySelector(".message-content").textContent : msg.textContent;
     
     if (msg.classList.contains("assistant-transcript") || msg.classList.contains("assistant")) {
         if (!lastAi) lastAi = text;
     } else if (msg.classList.contains("user-transcript") || msg.classList.contains("user")) {
         if (!lastUser) {
             lastUser = text;
             break; // Found the user message that prompted the AI
         }
     }
  }
